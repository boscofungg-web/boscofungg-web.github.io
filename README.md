...
- **`locked: true`** 代表要密碼先改得，見第八節。**而家只有港大課堂有**，
  其餘三個分類（新東方教學、其他／私人、可排課）任何人都改得，唔使密碼。
...
用途：**喺唔開放或者封鎖咗嘅日子手動開一格**。例如星期六本來 `openDays` 係 `false`
兼且 18:00 之後封鎖咗，自動計唔會出任何空檔 —— 手動擺一格「可排課」就開返嗰一格。

頁尾嘅「可排課 X 小時」會把自動同手動兩邊加埋一齊數，唔會數兩次。

**學生㩒手動嗰格 = 撳個空位。** 佢會開預約表格，時間已經填好，
同撳自動嗰啲空檔完全一樣。管理模式撳落去先係編輯（你要移動或者刪走佢）。

**手動「可排課」大過封鎖時段。** 你喺星期六 19:00 擺一格可排課，
就算星期六 18:00 之後係封鎖時段，學生一樣預約到嗰一格 ——
因為封鎖係「預設唔開放」，你明擺一格可排課就係「呢一格開放」。
同一日其他冇標記嘅時間照樣封鎖。
...
**例外：你明擺咗一格「可排課」喺封鎖時段入面，嗰一格照樣預約得到。**
封鎖係「呢段時間預設唔開放」，一格可排課就係「呢一格我特登開返」。
