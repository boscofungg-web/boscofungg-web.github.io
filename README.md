...
一般訪客可以新增、修改、刪除**任何**課堂 —— 改時間、改名、改色都得。
唯一做唔到嘅係**刪港大課堂**：個刪除掣根本唔會出現，佢哋改得但刪唔到。
...
| `openDays` | 邊幾日收新課，`[一,二,三,四,五,六,日]`。**而家全部 `true`** —— 所有冇課又冇封鎖嘅時間都自動變可排課 |
| `extraWindows` | 某日只開放部分時段。**而家係空 `{}`**，即唔限制。填咗嘅日子就淨係嗰幾段有可排課 |
| `lessonTypes` | 課堂類型（燈塔／中小／揚帆班課），見下面第九節 |
| `currency` | 銀碼前綴，而家係 `HK$` |
| `hideBefore` | 呢個日期之前嘅課堂唔會顯示。**而家係 `2026-09-01`** |
...
用途：**喺封鎖咗嘅時間手動開一格**。例如星期六 18:00 之後封鎖咗，
自動計唔會喺嗰度出空檔 —— 手動擺一格「可排課」就開返嗰一格。
...
**而家淨係靠封鎖時段擋。** `openDays` 全部 `true`、`extraWindows` 空咗，
即係話所有日子、所有冇課嘅時間都會自動變「可排課」，除非俾 `lockedWindows`、
`bookableFrom`/`bookableTo`（09:00–21:00）或者 `minSlot`（短過 1 小時唔計）擋住。
...
`categories` 入面加 `locked: true`，嗰個分類就只有管理模式先刪得到：
...
訪客開一堂港大課堂，個刪除掣唔會出現，下面會有一行字解釋點解，
叫佢有需要直接搵你。**改**就照樣改得 —— 改錯咗改返轉頭就得，刪咗就搵唔返。

呢個唔使密碼，所以亦冇「密碼寫喺公開檔案入面」嘅問題。
把關嘅係 `ownerKey`，同一句提醒：佢係門鎖唔係夾萬，`#你嘅密語` 寫喺公開 JS 入面，
識開 developer tools 嘅人搵得到。防手誤夠，防唔到有心人。真係要嚴謹就要 Firebase Authentication。
...
- **課堂類型** —— 燈塔／中小／揚帆班課。存喺 Firebase，**所有人揀得改得**。
- **時薪同銀碼** —— **只有管理模式先睇到**，而且只存喺你自己部機。

點解要分開：呢個 `index.html` 會放上公開 GitHub repo。時薪寫喺入面等於公開你嘅收費，
任何人 View Source 都睇到。所以時薪唔喺 `index.html`，而係喺網頁嘅「收費」面板入面設定，
存落 `localStorage` —— 唔上載 GitHub、唔入 Firebase、其他人攞唔到。

代價：**換部機、換瀏覽器、或者清咗瀏覽器資料，就要重新入一次時薪。** 三個數字，一分鐘。

### 用法

彈出視窗多咗「課堂類型」下拉選單，預設係「（唔計／未定）」。
**冇揀類型嘅課堂唔會計錢** —— 港大課堂、可排課、備課呢啲就係咁。

### 收入直接顯示喺日曆上（只有管理模式）

- **月曆**：每格右上角綠色金額 = 當日收入
- **週表**：星期標題下面顯示當日收入
- **頁尾**：跟住你睇緊嘅週／月，顯示合計鐘數同總收入

訪客乜都見唔到 —— 冇金額、冇收費掣、冇清除掣。

### 收費面板

管理模式頂部多咗「收費」掣，撳開之後：

- **時薪設定** —— 三個類型各自嘅每小時收費。改完即刻重算。
- **日子清單** —— 而家睇緊嗰個星期（或者月份）逐日一行，顯示鐘數同金額，逐日剔。
  預設剔晒有收入嗰幾日。三個快捷掣：全選／全部取消／只選有收入嘅日。
- **合計** —— 分類型列出「X 小時 × 時薪 = 金額」，最底係總數。
- 週表嘅星期標題會顯示當日收入（只有你睇到）。

轉去月檢視再撳「收費」，就變成成個月逐日一行，合計成個月。

### 清除日子（喺日曆度直接揀）

「月曆／週表」隔籬有個「清除日子」掣，只有管理模式見到。撳落去入清除模式：

1. 頂部出一條紅色工具列，顯示「已選 N 日 ・ M 堂」
2. **直接喺日曆上面撳日子**揀／取消 —— 月曆撳格仔，週表撳星期標題或者成條欄都得
3. 揀咗嘅日子紅色高亮
4. 撳兩次「清空選中日子」（第二次係確認）
5. 「離開清除模式」返回正常

清除模式之下撳唔到課堂、拖唔到新課，避免撳錯。

清走嘅方式：

- **單次課堂** → 直接刪除
- **每週重複嘅課堂** → 只係跳過嗰一日，**個系列同其他星期照舊**

即係話清空 9 月 14 號唔會殺死你逢星期一嗰幾堂，9 月 21 號照樣有。
用嚟清假期、颱風日、外遊嗰幾日最啱。

### 幾個要注意嘅位

**學生改得課堂類型。** 你話明要咁，我照做。但即係話有人可以把一堂「中小」改成「燈塔」，
你嘅收費計算就會跟住錯。埋單之前掃一眼個類型比較穩陣。想收返「只有管理模式改得類型」就講聲。

**未設時薪嗰個類型，合計嗰行會用紅字寫「未設時薪」**，金額當 0 計，唔會靜靜哋當冇嘢。

---

## 十、收入檢視

「月曆／週表」隔籬多咗個**「收入」**掣，只有管理模式先見到。訪客連個掣都冇。

裡面有：

- **本月收入大字**，加課堂數、總時數、平均時薪、有課日數
- **分類型** chips：燈塔／中小／揚帆班課 各自幾多錢、幾多鐘
- **月曆格**：每格得個日期同當日收入，加幾粒代表類型嘅色點。冇收入嘅日子淨係一個「—」
- **撳一日**下面出明細：逐堂時間、名、類型、金額。冇類型嘅課堂（港大課堂之類）會列出但標「唔計收入」
- **揀日子計數**：撳一下呢個掣就轉成揀日模式，喺月曆度剔／取消日子，
  上面個大數即刻變成「揀咗 N 日嘅收入」。想計某幾日拎幾多就用呢個
- **十二個月柱狀圖**：由當月數返轉頭 12 個月。撳一下柱就跳去嗰個月
- **匯出 CSV**：逐堂一行，最後一行合計。揀日模式下只匯出剔咗嗰幾日
- **時薪設定**：三個類型嘅每小時收費

收入檢視入面唔會出「清除日子」掣同顏色圖例 —— 嗰兩樣同收入無關。

### 時薪點解唔寫喺檔案入面

`index.html` 會放上公開 GitHub repo，時薪寫喺入面等於公開你嘅收費，
任何人 View Source 都睇到。所以時薪係喺網頁「時薪設定」入面填，
存落 `localStorage` —— 唔上載 GitHub、唔入 Firebase、其他人攞唔到。

代價：**換部機或者清咗瀏覽器資料就要再入一次**。三個數字，一分鐘。

### 收入係計出嚟嘅，唔會存落資料庫

Firebase 入面淨係存住課堂同佢個 `ltype`（類型）。金額係你部機上面用時薪即場乘出嚟。
即係話就算有人攞到你成個 Firebase 資料庫，都睇唔到你收幾多錢 —— 佢只會見到「燈塔 2 小時」。
...
| 睇收入 | 管理模式撳「收入」，或者㩒 `I`，見第十節 |
| 清一日 | 撳「清除日子」入清除模式，喺日曆揀日子，見第七節 |
